    ...
    try:
        ...
        with tqdm(total=(len(list(all_frame_joints.keys())) * 250)) as pchar:
            for fidx, (fno, frame_joints) in enumerate(tqdm(all_frame_joints.items())):
                # 画面内の位置
                right_arm_pos = get_vec3(frame_joints["joints"], f'right_arm')
                right_wrist_pos = get_vec3(frame_joints["joints"], f'right_wrist')
                left_arm_pos = get_vec3(frame_joints["joints"], f'left_arm')
                left_wrist_pos = get_vec3(frame_joints["joints"], f'left_wrist')

                # 中央0からの位置とする
                right_arm_pos.setX(right_arm_pos.x() - 0.5)
                right_wrist_pos.setX(right_wrist_pos.x() - 0.5)
                left_arm_pos.setX(left_arm_pos.x() - 0.5)
                left_wrist_pos.setX(left_wrist_pos.x() - 0.5)
                # 上0からの位置とする
                right_arm_pos.setY(right_arm_pos.y() * -1)
                right_wrist_pos.setY(right_wrist_pos.y() * -1)
                left_arm_pos.setY(left_arm_pos.y() * -1)
                left_wrist_pos.setY(left_wrist_pos.y() * -1)

                # 手首の中点
                wrist_center_pos = (right_wrist_pos + left_wrist_pos) / 2
                ...
                wrist_cetnter_vec = (now_right_wrist_vec + now_left_wrist_vec) / 2

                for direction_ja, direction_en, direction_sign, effector_links, ik_links, now_arm_vec, now_wrist_vec, reverse_arm_vec, reverse_wrist_vec, wrist_pos, arm_pos, reverse_wrist_pos, reverse_arm_pos in [ \
                    ('右', 'right', 1, right_wrist_links, right_ik_links, now_right_arm_vec, now_right_wrist_vec, now_left_arm_vec, now_left_wrist_vec, right_wrist_pos, right_arm_pos, left_wrist_pos, left_arm_pos), \
                    ('左', 'left', -1, left_wrist_links, left_ik_links, now_left_arm_vec, now_left_wrist_vec, now_right_arm_vec, now_right_wrist_vec, left_wrist_pos, left_arm_pos, right_wrist_pos, right_arm_pos)]:
                    logger.debug('{0} ------------------', fno)

                    # 比率を腕位置から求める
                    wrist_center_ratio = (wrist_center_pos - arm_pos) / (reverse_arm_pos - arm_pos)
                    # 腕から中点位置を求める
                    mmd_wrist_center_vec = now_arm_vec + ((reverse_arm_vec - now_arm_vec) * wrist_center_ratio)
                    # 中点からの差分で手首の位置を求め直す
                    final_effector_pos = now_wrist_vec - wrist_cetnter_vec + mmd_wrist_center_vec
                    ...
                    for ik_cnt in range(50):
                        pchar.update(1)
                        MServiceUtils.calc_IK(model, effector_links, motion, fno, final_effector_pos, ik_links, max_count=1)
                        ...
                        diff_pos = MVector3D() if final_effector_pos == MVector3D() else final_effector_pos - now_effector_pos

                        if prev_diff == MVector3D() or (prev_diff != MVector3D() and diff_pos.length() < prev_diff.length()):
                            if diff_pos.length() < 0.05:
                                logger.debug("☆腕ＩＫ変換成功({0}): f: {1}({2}), 指定 [{3}], 現在[{4}], 差異[{5}({6})]", ik_cnt, fno, effector_bone_name, \
                                            final_effector_pos.to_log(), now_effector_pos.to_log(), diff_pos.to_log(), diff_pos.length())
                                ...
                                # そのまま終了
                                break
                            elif (prev_diff == MVector3D() and diff_pos.length() < 0.2) or diff_pos.length() < prev_diff.length():
                                logger.debug("☆腕ＩＫ変換ちょっと失敗採用({0}): f: {1}({2}), 指定 [{3}], 現在[{4}], 差異[{5}({6})]", ik_cnt, fno, effector_bone_name, \
                                            final_effector_pos.to_log(), now_effector_pos.to_log(), diff_pos.to_log(), diff_pos.length())
                                ...
                                if prev_diff == diff_pos or np.count_nonzero(np.where(np.abs(diff_pos.data()) > 0.001, 1, 0)) == 0:
                                ...
                                logger.debug("★腕ＩＫ変換ちょっと失敗不採用({0}): f: {1}({2}), 指定 [{3}], 現在[{4}], 差異[{5}({6})]", ik_cnt, fno, effector_bone_name, \
                                            final_effector_pos.to_log(), now_effector_pos.to_log(), diff_pos.to_log(), diff_pos.length())
                            ...
                            logger.debug("★腕ＩＫ変換失敗({0}): f: {1}({2}), 指定 [{3}], 現在[{4}], 差異[{5}({6})]", ik_cnt, fno, effector_bone_name, \
                                        final_effector_pos.to_log(), now_effector_pos.to_log(), diff_pos.to_log(), diff_pos.length())
                    ...
                    for link_name in list(ik_links.all().keys())[1:]:
                        ...
                        logger.debug("確定bf: {0} [{1}]", link_name, bf.rotation.toEulerAngles().to_log())
                        motion.regist_bf(bf, link_name, fno)
                    
        # with tqdm(total=(len(list(all_frame_joints.keys())) * 50 * 3 * 3)) as pchar:
        #     for fidx, (fno, frame_joints) in enumerate(all_frame_joints.items()):
        #         # 画面内の位置
        #         right_arm_pos = get_vec3(frame_joints["joints"], f'right_arm')
        #         right_wrist_pos = get_vec3(frame_joints["joints"], f'right_wrist')
        ...
        #         for direction_ja, direction_en, direction_sign, effector_links, now_arm_vec, now_wrist_vec, reverse_arm_vec, reverse_wrist_vec, wrist_pos, arm_pos, reverse_wrist_pos, reverse_arm_pos in [ \
        #             ('右', 'right', 1, right_wrist_links, now_right_arm_vec, now_right_wrist_vec, now_left_arm_vec, now_left_wrist_vec, right_wrist_pos, right_arm_pos, left_wrist_pos, left_arm_pos), \
        #             ('左', 'left', -1, left_wrist_links, now_left_arm_vec, now_left_wrist_vec, now_right_arm_vec, now_right_wrist_vec, left_wrist_pos, left_arm_pos, right_wrist_pos, right_arm_pos)]:
        #             logger.debug('{0} ------------------', fno)

        #             # 比率を腕位置から求める
        #             wrist_center_x_ratio = (wrist_center_pos.x() - arm_pos.x()) / (reverse_arm_pos.x() - arm_pos.x())
        #             # 腕から中点位置を求める
        #             mmd_wrist_center_x = (reverse_arm_vec.x() - now_arm_vec.x()) * wrist_center_x_ratio
        #             # 中点からの差分で手首の位置を求め直す
        #             final_effector_pos = MVector3D(now_wrist_vec.x() + (wrist_cetnter_vec.x() - mmd_wrist_center_x), now_wrist_vec.y(), now_wrist_vec.z())
        #             # Xの差異
        #             effector_diff_pos = final_effector_pos - now_wrist_vec

        #             logger.debug("{0}({1}): 目標手首位置: {2}, 現在手首位置: {3}, 差異: {4}({5})", fno, direction_ja, final_effector_pos.to_log(), \
        #                          now_wrist_vec.to_log(), effector_diff_pos.to_log(), effector_diff_pos.length())

        #             # X方向の調整
        #             didx = 0
        #             while effector_diff_pos.length() > 0.01 and didx < 50:
        #                 pchar.update(1)

        #                 shoulder_bf = motion.calc_bf(f'{direction_ja}肩', fno)
        #                 cancel_degree = effector_diff_pos.length() * 2

        #                 # 肩を少し反らせる
        #                 sqq = MQuaternion.fromEulerAngles(0, cancel_degree * direction_sign, 0)
        #                 shoulder_bf.rotation = sqq * shoulder_bf.rotation
        #                 motion.regist_bf(shoulder_bf, shoulder_bf.name, shoulder_bf.fno)

        #                 new_wrist_3ds_dic = MServiceUtils.calc_global_pos(model, effector_links, motion, fno)
        #                 new_wrist_vec = new_wrist_3ds_dic[f'{direction_ja}手首']

        #                 # 最新の差分
        #                 new_effector_diff_pos = final_effector_pos - new_wrist_vec

        #                 logger.debug("{0}({1}-{2}): 肩: {3}, 手首位置: {4}, 差異: {5}({6}), 角度: {7}", fno, direction_ja, didx, shoulder_bf.rotation.toEulerAngles4MMD().to_log(), \
        #                              new_wrist_vec.to_log(), new_effector_diff_pos.to_log(), new_effector_diff_pos.length(), cancel_degree)
                        
        #                 # 充分に近くなったか
        #                 if new_effector_diff_pos.length() <= 0.01:
        #                     effector_diff_pos = new_effector_diff_pos
        #                     break
                        
        #                 if new_effector_diff_pos.length() > effector_diff_pos.length():
        #                     # 前より離れた場合、戻す
        #                     sqq = MQuaternion.fromEulerAngles(0, cancel_degree * direction_sign * -1, 0)
        #                     shoulder_bf.rotation = sqq * shoulder_bf.rotation
        #                     motion.regist_bf(shoulder_bf, shoulder_bf.name, shoulder_bf.fno)
        #                 else:
        #                     # 前より近付いている場合置き換え
        ...
        #                     arm_bf = motion.calc_bf(f'{direction_ja}腕', fno)

        #                     # 腕を少し反らせる
        #                     aqq = MQuaternion.fromEulerAngles(0, cancel_degree * direction_sign, 0)
        #                     arm_bf.rotation = aqq * arm_bf.rotation
        #                     motion.regist_bf(arm_bf, arm_bf.name, arm_bf.fno)

        #                     now_wrist_3ds_dic = MServiceUtils.calc_global_pos(model, effector_links, motion, fno)
        #                     now_wrist_vec = now_wrist_3ds_dic[f'{direction_ja}手首']

        #                     # 最新の差分
        #                     new_effector_diff_pos = final_effector_pos - now_wrist_vec

        #                     logger.debug("{0}({1}-{2}-{3}): 腕: {4}, 手首位置: {5}, 差異: {6}({7}), 角度{8}", fno, direction_ja, didx, aidx, arm_bf.rotation.toEulerAngles4MMD().to_log(), \
        #                                 now_wrist_vec.to_log(), new_effector_diff_pos.to_log(), new_effector_diff_pos.length(), cancel_degree)

        #                     # 充分に近くなった場合終了
        #                     if abs(new_effector_diff_pos.x()) <= 0.01:
        #                         effector_diff_pos = new_effector_diff_pos
        #                         break
                                    
        #                     # 前より離れた場合、戻して終了
        #                     if abs(new_effector_diff_pos.x()) > abs(effector_diff_pos.x()):
        #                         aqq = MQuaternion.fromEulerAngles(0, cancel_degree * direction_sign * -1, 0)
        #                         arm_bf.rotation = aqq * arm_bf.rotation
        #                         motion.regist_bf(arm_bf, arm_bf.name, arm_bf.fno)
        #                         break
        #                     else:
        ...
        #                     elbow_bf = motion.calc_bf(f'{direction_ja}ひじ', fno)

        #                     # ひじを少し曲げる
        #                     eqq = MQuaternion.fromEulerAngles(0, 0, cancel_degree)
        #                     elbow_bf.rotation = elbow_bf.rotation * eqq
        #                     motion.regist_bf(elbow_bf, elbow_bf.name, elbow_bf.fno)
        ...
        #                     logger.debug("{0}({1}-{2}-{3}): ひじ: {4}, 手首位置: {5}, 差異: {6}({7}), 角度{8}", fno, direction_ja, didx, eidx, elbow_bf.rotation.toEulerAngles4MMD().to_log(), \
        #                                 now_wrist_vec.to_log(), new_effector_diff_pos.to_log(), new_effector_diff_pos.length(), cancel_degree)

        #                     # 充分に近くなった場合終了
        #                     if abs(new_effector_diff_pos.y()) <= 0.01:
        #                         effector_diff_pos = new_effector_diff_pos
        #                         break
                                    
        #                     # 前より離れた場合、戻して終了
        #                     if abs(new_effector_diff_pos.y()) > abs(effector_diff_pos.y()):
        #                         eqq = MQuaternion.fromEulerAngles(0, 0, cancel_degree * -1)
        #                         elbow_bf.rotation = elbow_bf.rotation * eqq
        #                         motion.regist_bf(elbow_bf, elbow_bf.name, elbow_bf.fno)
        #                         effector_diff_pos = new_effector_diff_pos
        #                         break
